...
func TestUpgradeCheckerPreservesLastGoodConfigAndRecovers(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(path, []byte(`{"upgrades":[{"height":100,"name":"v2","status":"scheduled","source":"on-chain"}]}`), 0o600))
	...
	assert.Equal(t, []Upgrade{{Height: 100, Name: "v2", Status: UpgradeScheduled, Source: OnChainUpgrade}}, checker.Snapshot().Upgrades)
...
func TestUpgradeCheckerWatchesConfigMapSymlinkReplacement(t *testing.T) {
	...
	writeVersion("..2026_02", `{"upgrades":[{"height":200,"name":"v3","status":"scheduled","source":"on-chain"}]}`)
