...
const (
	...
	ReasonConsensusKeyReservationBlocked   = "ConsensusKeyReservationBlocked"
	ReasonConsensusKeyReservationReleased  = "ConsensusKeyReservationReleased"
	ReasonConsensusKeyReservationRecovered = "ConsensusKeyReservationRecovered"
)
...
type VolumeSpec struct {
	...
	// Deprecated compatibility field. Root deletion is controlled by
	// .spec.deletionPolicy.dataVolumes; this field is not used for cleanup.
	// +optional
	DeleteWithNode *bool `json:"deleteWithNode,omitempty"`
}

// DeletionPolicyType controls what happens to operator-generated durable resources when their
// ChainNode or ChainNodeSet root is deleted.
// +kubebuilder:validation:Enum=Retain;Delete
type DeletionPolicyType string

const (
	// DeletionPolicyRetain preserves attributed durable resources after the owning root is deleted.
	DeletionPolicyRetain DeletionPolicyType = "Retain"
	// DeletionPolicyDelete removes attributed durable resources after every signing workload is stopped.
	DeletionPolicyDelete DeletionPolicyType = "Delete"
)

// DeletionPolicy configures deletion of durable resources generated by Cosmopilot. Every field
// defaults to Retain; deletion always requires an explicit Delete value.
type DeletionPolicy struct {
	// DataVolumes controls operator-generated main and additional node PVCs and, for a ChainNodeSet,
	// Cosmoseed `data-<set>-seed-<ordinal>` PVCs.
	// +optional
	// +kubebuilder:default=Retain
	DataVolumes *DeletionPolicyType `json:"dataVolumes,omitempty"`

	// GeneratedKeys controls operator-generated node, consensus, account, genesis-validator, and
	// Cosmoseed key Secrets. Existing user-provided Secrets are never classified from their name alone.
	// +optional
	// +kubebuilder:default=Retain
	GeneratedKeys *DeletionPolicyType `json:"generatedKeys,omitempty"`

	// CosmosignerState controls operator-generated Cosmosigner Raft-state PVCs.
	// +optional
	// +kubebuilder:default=Retain
	CosmosignerState *DeletionPolicyType `json:"cosmosignerState,omitempty"`
}
...
type ExportTarballConfig struct {
	...
	// (concurrentJobs + 1) of memory for GCS, or of ephemeral storage for the S3 part spool.
	// +optional
	Resources *corev1.ResourceRequirements `json:"resources,omitempty"`
}
