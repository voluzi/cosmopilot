...
func (chainNode *ChainNode) ValidateCreate(_ context.Context, obj *ChainNode) (warnings admission.Warnings, err error) {
	...
	// (e.g. <set>-validator, or <base>-<ordinal> under a -signer/-cg/-seed group). Those
	// names are produced by the operator, not a user, so exempt them from the reserved-name
	// checks; the ChainNodeSet webhook already guards the names it derives.
	if !isControlledByChainNodeSet(obj) {
