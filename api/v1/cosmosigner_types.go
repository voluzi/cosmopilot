...
type CosmosignerStatus struct {
	...
	// or — for a SOFTWARE sentry signer whose privateKeySecret is listed in an ACTIVE (non-zero-instance)
	// validator's spec.validator.init.genesisValidators or spec.nodes[].validator.init.genesisValidators
	...
	// (admitted) apart from one introduced afterwards (rejected unless the backend provably imports the
	// registered key), and reject a later key change/removal of a genesis-registered software sentry
	// signer. Absent for a signer added after establishment.
	// +optional
