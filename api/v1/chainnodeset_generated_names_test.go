...
// A gRPC Ingress is backed by a gRPC-only Service named like the Ingress ("<route>-grpc" globally,
// "<child>-grpc" for individual ingresses), so that Service name is claimed whenever gRPC is exposed.
func TestValidateGrpcOnlyServiceNameCollisions(t *testing.T) {
	routeVsGateway := func(grpc bool, servicesOnly *bool) *ChainNodeSet {
		return &ChainNodeSet{
			ObjectMeta: metav1.ObjectMeta{Name: "cs"},
			Spec: ChainNodeSetSpec{
				Ingresses:     []GlobalIngressConfig{{Name: "x", EnableGRPC: grpc, ServicesOnly: servicesOnly}},
				GatewayRoutes: []GlobalGatewayConfig{{Name: "x-grpc"}},
			},
		}
	}
	err := routeVsGateway(true, nil).validateServiceNameCollisions(nil)
	require.Error(t, err)
	...
	childVsGroup := func(grpc bool) *ChainNodeSet {
		return &ChainNodeSet{
			ObjectMeta: metav1.ObjectMeta{Name: "cs"},
			Spec: ChainNodeSetSpec{Nodes: []NodeGroupSpec{
				{Name: "foo", Instances: ptr.To(1), IndividualIngresses: &IngressConfig{EnableGRPC: grpc}},
				{Name: "foo-0-grpc"},
			}},
		}
	}
	err = childVsGroup(true).validateServiceNameCollisions(nil)
	require.Error(t, err)
	require.Contains(t, err.Error(), "cs-foo-0-grpc")
	require.NoError(t, childVsGroup(false).validateServiceNameCollisions(nil))
}
