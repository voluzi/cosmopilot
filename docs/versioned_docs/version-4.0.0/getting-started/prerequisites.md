...
:::tip
If your image requires different security settings, you can override the security context. See [Security Context Overrides](../usage/node-config#security-context-overrides) for details.
:::

## **Chain Compatibility**

Cosmopilot works with chains built on the Cosmos SDK. Before deploying, verify your chain is compatible by checking the [Chain Compatibility](../getting-started/chain-compatibility) page, which lists:
...
Ensure all dependencies are installed, your cluster is correctly configured, and your chain is compatible before proceeding to the [Installation](../getting-started/installation) page.