...
This will ensure that `EVM` `RPC` ports will be added to the node's service and will be available when [exposing the endpoints](../usage/exposing-endpoints).
...
For configuring additional volumes, see [Persistence and Backups - Additional Volumes](../usage/persistence-and-backup#additional-volumes).
...
Please refer to [SidecarSpec](../reference/crds#sidecarspec) for full specification on all available fields.
