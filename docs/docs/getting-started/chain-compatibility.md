...
These commands are only required when [initializing a new network](../usage/initializing-new-network).
...
This command is only required when running a [validator node](../usage/validator).
