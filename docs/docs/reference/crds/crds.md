...
| readyInstances | Indicates how many of the ChainNode instances on this ChainNodeSet are currently ready, i.e. their phase is Running, Syncing or Snapshotting. A value below .status.instances means the nodeset is degraded even while .status.phase is Running. | int | false |
