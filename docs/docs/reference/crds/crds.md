...
| Field | Description | Scheme | Required |
| ----- | ----------- | ------ | -------- |
| dataVolumes | DataVolumes controls operator-generated main and additional node PVCs and, for a ChainNodeSet, Cosmoseed `data-<set>-seed-<ordinal>` PVCs. | *[DeletionPolicyType](#deletionpolicytype) | false |
| generatedKeys | GeneratedKeys controls operator-generated node, consensus, account, genesis-validator, and Cosmoseed key Secrets. Existing user-provided Secrets are never classified from their name alone. | *[DeletionPolicyType](#deletionpolicytype) | false |
| cosmosignerState | CosmosignerState controls operator-generated Cosmosigner Raft-state PVCs. | *[DeletionPolicyType](#deletionpolicytype) | false |

[Back to Custom Resources](#custom-resources)

#### DeletionPolicyType

DeletionPolicyType controls whether an attributed durable resource is preserved or removed when its
`ChainNode` or `ChainNodeSet` root is deleted. Valid values are `Retain` and `Delete`. The default is
`Retain`; destructive cleanup requires an explicit `Delete` value.
