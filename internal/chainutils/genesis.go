...
func (a *App) buildGenesisPod(
		...
		Spec: corev1.PodSpec{
			...
			InitContainers: []corev1.Container{
				...
				},
				{
					...
					Env:             a.appEnv("add-validator-account"),
					VolumeMounts:    []corev1.VolumeMount{dataVolumeMount},
					SecurityContext: k8s.RestrictedSecurityContext(),
				},
			},
	...
	for i, acc := range params.Accounts {
		pod.Spec.InitContainers = append(pod.Spec.InitContainers, corev1.Container{
			...
			Env:             a.appEnv(fmt.Sprintf("add-account-%d", i)),
			VolumeMounts:    []corev1.VolumeMount{dataVolumeMount},
			SecurityContext: k8s.RestrictedSecurityContext(),
		})
	}
	...
	for i, ev := range extraValidators {
		...
		pod.Spec.InitContainers = append(pod.Spec.InitContainers,
			...
			corev1.Container{
				...
				Env:             a.appEnv(fmt.Sprintf("load-account-%d", idx)),
				Stdin:           true,
				StdinOnce:       true,
				VolumeMounts:    []corev1.VolumeMount{dataVolumeMount},
				SecurityContext: k8s.RestrictedSecurityContext(),
			...
			corev1.Container{
				...
				Env:             a.appEnv(fmt.Sprintf("add-validator-account-%d", idx)),
				VolumeMounts:    []corev1.VolumeMount{dataVolumeMount},
				SecurityContext: k8s.RestrictedSecurityContext(),
			},
			corev1.Container{
				...
				Env:             a.appEnv(fmt.Sprintf("gentx-%d", idx)),
				VolumeMounts:    []corev1.VolumeMount{dataVolumeMount},
				SecurityContext: k8s.RestrictedSecurityContext(),
			},
		)
