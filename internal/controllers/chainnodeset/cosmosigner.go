...
func (r *Reconciler) signerImportSourcePending(nodeSet *appsv1.ChainNodeSet, s appsv1.ResolvedSigner) bool {
	...
	// POST-GENESIS (and non-init): judge by the PER-INSTANCE derived config. The group's keys were created
	// during bootstrap, so they exist now; a source that is still missing will NOT be recreated.
	//   - an explicit user-supplied key is never (re)generated by any flow — terminal;
	//   - only an instance that GENERATES its own key (its derived Init/createValidator) is pending until
	//     its pubkey is recorded — a nonzero instance of a genesis-init group has Init cleared, so it is
	//     terminal (its pre-created key must exist) rather than waiting forever for a pubkey it can never
	//     record while its source Secret is missing.
