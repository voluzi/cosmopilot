...
func (r *Reconciler) signerImportSourcePending(nodeSet *appsv1.ChainNodeSet, s appsv1.ResolvedSigner) bool {
	...
	// A genesis-init group GENERATES its consensus keys during bootstrap (ensureValidator creates ALL of
	// its instance keys together, into each instance's PrivateKeySecret — explicit or default), so an init
	// validator's source key is pending until that flow records the instance's pubkey. This takes priority
	// over the explicit-key "user-supplied" shortcut below, which must NOT apply to a generated genesis
	// key, and over the per-instance derived config (a nonzero instance has Init cleared but is still
	// generated by the group). Pre-genesis nothing has run yet, so it is pending.
	if groupValidator != nil && groupValidator.Init != nil {
		if nodeSet.Status.ChainID == "" {
			return true
		}
		return !pubKeyRecorded()
	}
	// Non-init validators: an explicit user-supplied key is never generated by any controller flow.
