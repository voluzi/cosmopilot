...
func TestAuthoritativeSDKHaltLog(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{name: "later valid candidate wins after wrong height", logs: logTimestamp + ` {"level":"info","height":99,"time":0,"_msg":"halting node per configuration"}` + "\n" + validJSON, want: true},
		{name: "later valid candidate wins after stale record", logs: "2026-09-21T11:59:59Z " + strings.SplitN(validJSON, " ", 2)[1] + validPlain, want: true},
		{name: "later valid candidate wins after malformed record", logs: logTimestamp + ` {"height":100,"time":0,"_msg":"halting node per configuration"` + "\n" + validPlain, want: true},
		{name: "embedded plain prefix is rejected", logs: logTimestamp + " noise I[2026-09-21|12:00:30.123] halting node per configuration height=100 time=0\n"},
