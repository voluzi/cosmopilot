...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if r.opts.DisableWebhooks {
		...
		// Child ChainNodes generated by a ChainNodeSet legitimately carry reserved suffixes
		// (e.g. <set>-validator, or <base>-<ordinal> under a -signer/-cg/-seed group), so exempt
		// them just like the admission webhook does.
		if !chainNode.IsControlledByChainNodeSet() {
			...
			if err := appsv1.ValidateReservedStatefulChildName(chainNode.GetName(), chainNode.Status.NodeID == ""); err != nil {
				logger.Error(err, "spec is invalid")
				r.recorder.Eventf(chainNode, corev1.EventTypeWarning, appsv1.ReasonInvalid, "spec is invalid: %v", err)
				return ctrl.Result{}, err
			}
	...
	if stopNodeSnapshotHoldsPod(chainNode) {
		logger.Info("exiting reconcile cycle while stop-node snapshot is pending or in progress")
